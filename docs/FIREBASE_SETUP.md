...
- Perfil do admin pode salvar nome, empresa e uma foto real comprimida no proprio documento do usuario.
...
- OTA funciona no plano gratis: o `.bin` fica hospedado fora do Firebase, por exemplo no GitHub Releases, e o painel grava URL, SHA-256, tamanho e versao no Firestore.

## Observacao sobre OTA

O painel admin grava os metadados do firmware no Firestore e gera o manifesto na tela. Para usar GitHub Releases, cole um link como `https://github.com/Isaqueeln11/valvula-pro/releases/tag/v2.5.5`; o painel tenta encontrar o primeiro asset `.bin` da release automaticamente. Para um equipamento embarcado baixar automaticamente um JSON publico e sempre atualizado, o proximo passo ideal e publicar uma API/Cloud Function que leia `firmwareReleases` e responda `/ota/manifest.json`.
